...
namespace WebGLSupport
{
	internal class WebGLInputPlugin
	{
		...
		[DllImport("__Internal")]
		public static extern int WebGLInputCreate(string canvasId, int x, int y, int width, int height, int fontsize,
	...
	{
		private static Dictionary<int, WebGLInput> instances = new();
		...
		public void OnSelect()
		{
			...
			var isPassword = input.contentType == ContentType.Password;

			// var fontSize = Mathf.Max(14, input.fontSize); // limit font size : 14 !!
			var fontSize = 20;
			// モバイルの場合、強制表示する
			var isHidden = !(showHtmlElement || Application.isMobilePlatform);
			id = WebGLInputPlugin.WebGLInputCreate(CanvasId, rect.x, rect.y, rect.width, rect.height, fontSize,
				input.text, input.placeholder, input.lineType != LineType.SingleLine, isPassword, isHidden,
		...
		{
			private static List<WebGLInput> inputs = new();
